...
import os, math, string, sys, os.path, re, requests, StringIO
...
elements={'p':(1,1),'n':(0,1),'H':(1,1),'HE':(2,4),'LI':(3,7),'BE':(4,9),'B':(5,10),
	'C':(6,12),'N':(7,14),'O':(8,16),'F':(9,19),'NE':(10,20),'NA':(11,23),'MG':(12,24),'AL':(13,27),'SI':(14,28),
        ...
        'LU':(71,175),'HF':(72, 178),'TA':(73,181),'W':(74,184),'RE':(75,186),'OS':(76,190),'IR':(77,192),'PT':(78,195),
...
def main(Z_str, A_str):

  Z = int(Z_str)
  A = int(A_str)

  # The NNDC ENSDF database uses this number to
  # index the datasets, together with the NUCID.
  # The "+ 1" at the end represents the "ADOPTED LEVELS, GAMMAS" table.
  dataset_number = 1000000*A + 1000*Z + 1

  # Construct the NUCID by finding the appropriate element symbol for the given
  # proton number Z
  element_symbol = next(key for key,value in elements.iteritems()
    if value[0] == Z)

  dataset_id = str(dataset_number) + ',' + str(A) + element_symbol

  my_data = {
    'chooseit': 'ENSDF text format',
    'dbclass': 'ensdf',
    'datasetcheck': dataset_id, # e.g., 208Pb: '208082001,208PB'
    'pagesource': 'singular'
  }

  response = requests.post('https://www.nndc.bnl.gov/ensdf/EnsdfDispatcherServlet', data=my_data)

  soup = BeautifulSoup( response.content, 'lxml' )

  ensdf_text = soup.find('pre').text
